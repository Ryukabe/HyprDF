...
apply_layout() {
    ...
    # Use 'ln -sf' to create or overwrite the symbolic links
    # -s: symbolic link
    # -f: force (removes existing file/link to replace it)
    [ -n "$config_file" ] && ln -sf "$config_file" "$CONFIG"
    [ -n "$style_file" ]  && ln -sf "$style_file" "$STYLECSS"
