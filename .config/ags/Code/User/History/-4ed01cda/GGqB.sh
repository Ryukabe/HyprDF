...
if [ -n "$WALLPAPER" ] && [ -f "$WALLPAPER" ]; then
    awww img "$WALLPAPER" --transition-type "any" --transition-fps 50 > /dev/null 2>&1
    # Also update hyprlock symlink  --transition-step 255
...
OBSIDIAN_THEME_FILE="$THEME_DIR/obsidian/theme-name"

# Add all your vault paths here
VAULT_PATHS=(
    "$HOME/Documents/03.Resource/Alvi's Vault"
)

if [ -f "$OBSIDIAN_THEME_FILE" ]; then
    OBSIDIAN_THEME=$(cat "$OBSIDIAN_THEME_FILE")
...
with open('$APPEARANCE', 'r') as f:
    a = json.load(f)
a['cssTheme'] = '$OBSIDIAN_THEME'
with open('$APPEARANCE', 'w') as f:
    json.dump(a, f, indent=2)
"
            echo -e "${GREEN}   Obsidian theme set in $VAULT${NC}"
        else
            echo -e "${YELLOW}   appearance.json not found in $VAULT${NC}"
        fi
    done
else
    echo -e "${YELLOW}   Obsidian theme file not found. Skipping.${NC}"
fi
echo ""
