...
case "$THEME" in
    ayu|everforest|gruvbox|catppuccin|tokyonight|monochrom|kanagawa|dragon|nord|noir|nightfox|carbonfox|rosepine|vesper|vira-palenight|horizon|material-you|anime)
...
NV_CONFIG="$HOME/.config/nvim/lua/chadrc.lua"

if [ -f "$NV_CONFIG" ]; then
    cp "$THEME_DIR/nvim/chadrc.lu" "$HOME/.config/nvim/lua/chadrc.lua" > /dev/null 2>&1
    echo -e "${GREEN}   Neovim theme updated to $THEME${NC}"
else
    echo -e "${YELLOW}   Neovim config not found at $NV_CONFIG${NC}"
fi
