...
# 3. Build list and pipe directly into Rofi
# Use file:// URI for the icon so Rofi can render the actual image thumbnail
selected=$(find "$WALL_DIR" -maxdepth 1 -type f \
    \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" \) \
    | sort \
    | while IFS= read -r full_path; do
        fname=$(basename "$full_path")
        printf $'%s\0icon\x1ffile://%s\n' "$fname" "$full_path"
    done \
    | rofi -dmenu -i -no-custom -show-icons \
           -p "Select Wallpaper" \
           -config "$HOME/.config/rofi/wall.rasi")

# 4. Apply selection and save state
if [ -n "$selected" ]; then
