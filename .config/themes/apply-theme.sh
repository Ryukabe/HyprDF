...
if [ -n "$WALLPAPER" ] && [ -f "$WALLPAPER" ]; then
    awww img "$WALLPAPER" --transition-type "any" --transition-fps 60 > /dev/null 2>&1 #--transition-step 255
    ln -sf "$WALLPAPER" ~/.config/hypr/hyprlock/wallpaper
    ln -sf "$WALLPAPER" ~/.config/rofi/wallpaper/current_wallpaper
else
    echo -e "${YELLOW}   Could not set wallpaper${NC}"
fi
echo ""

# ── material-you: run matugen from saved wallpaper ────────────────────────────
if [[ "$THEME" == "material-you" ]] && [ -n "$WALLPAPER" ] && [ -f "$WALLPAPER" ]; then
    echo -e "${CYAN}→ Running matugen for material-you theme...${NC}"
    matugen --config "$HOME/.config/matugen/config.toml" --source-color-index 0 image "$WALLPAPER" > /dev/null 2>&1
    sleep 0.5  # wait for matugen to finish writing all files
    echo -e "${GREEN}   matugen colors written to custom/ folders${NC}"
    echo ""
fi
# ─────────────────────────────────────────────────────────────────────────────
...
#GTK3_DST="$HOME/.config/gtk-3.0"
#
#if [[ -d "$GTK3_SRC" ]]; then
#    echo -e "${CYAN}→ Linking GTK3 theme files...${NC}"
#    mkdir -p "$GTK3_DST"
#    ln -sf "$GTK3_SRC/gtk.css" "$GTK3_DST/gtk.css"
#    ln -sf "$GTK3_SRC/gtk-dark.css" "$GTK3_DST/gtk-dark.css"
#    ln -sfn "$GTK3_SRC/assets" "$GTK3_DST/assets"
#else
#    echo -e "${YELLOW}→ No GTK3 theme files found in $GTK3_SRC. Skipping.${NC}"
#fi
#echo ""
...
# Neovim Theme
echo -e "${CYAN}→ Syncing Neovim theme...${NC}"
if cp "$THEME_DIR/nvim/chadrc.lua" "$HOME/.config/nvim/lua/chadrc.lua" > /dev/null 2>&1; then
    echo -e "${GREEN}   Neovim theme updated to $THEME${NC}"
else
    echo -e "${YELLOW}   Neovim config not found or copy failed${NC}"
fi
echo ""
...
# Obsidian theme
#echo -e "${CYAN}→ Applying Obsidian theme...${NC}"
#cp "$THEME_DIR/obsidian/appearance.json" "$HOME/03.Resource/Vault/.obsidian/appearance.json" > /dev/null 2>&1
#echo ""

# VSCode theme
echo -e "${CYAN}→ Applying VSCode theme...${NC}"
cp "$THEME_DIR/vscode/settings.json" "$HOME/.config/Code/User/settings.json" > /dev/null 2>&1
echo ""
...
notify-send "Theme Applied" "Successfully switched to $THEME" -t 4500